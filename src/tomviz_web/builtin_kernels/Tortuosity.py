...
def get_distance_function(method):
    ...
    if method == DistanceMethod.ChessBoard:
    ...
    if method == DistanceMethod.CityBlock:
...
def volume_to_graph(volume, phase, method, update_progress=None):
    ...
    for i, (node_idx, node_coord) in enumerate(inv_node_map.items()):
        ...
        for delta_coord, delta_dist in zip(
            delta_coordinates, delta_distances, strict=False
        ):
            neighbor_coord = tuple(
                c + d for c, d in zip(node_coord, delta_coord, strict=False)
            )
            ...
            if delta_dist is not None:
                ...
                edge_key = (min(node_idx, neighbor_idx), max(node_idx, neighbor_idx))
...
def edges_to_sparse_matrix(edges, aux_edges, aux_node_idx, n_nodes, ndim):
    ...
    sparse_edge_matrix = csr_matrix(
        (data, (row, col)), shape=(total_nodes, total_nodes)
    )
...
def get_update_progress_fn(progress, stage):
    ...
    if stage == OperatorStages.GraphTraversal:

        def update_progress(value):
            progress.value = round(
                progress.maximum * (value * OTHER_FRACTION + GRAPH_GENERATION_FRACTION)
            )
...
class TortuosityOperator(tomviz.operators.CancelableOperator):
    ...
    def transform(
        ...
        if save_to_file and not os.access(output_folder, os.W_OK):
            ...
            save_to_file = False
            warnings.warn("Unable to write to destination folder %s" % output_folder)
        ...
        node_map, inv_node_map, node_map_array, edges, aux_edges = volume_to_graph(
            scalars, phase, distance_method, graph_generation_update_progress_fn
        )
        ...
        for i, direction in enumerate(propagation_directions):
            ...
            csgraph = edges_to_sparse_matrix(
                edges, aux_edges, direction.value, len(inv_node_map), scalars.ndim
            )

            graph_traversal_update_progress_fn(i / n_directions + 0.33)

            dist_matrix = dijkstra(csgraph, directed=False, indices=direction.value)
