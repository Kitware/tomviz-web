...
class ArtifactsTVOperator(tomviz.operators.CancelableOperator):
    def transform(self, dataset, Niter=100, a=0.1, wedgeSize=5, kmin=5, theta=0):
        """
        ...
        mask[
            np.where((phi >= (theta - dtheta / 2)) & (phi <= (theta + dtheta / 2)))
        ] = 0
        mask[
            np.where(
                (phi >= (np.pi + theta - dtheta / 2))
                & (phi <= (np.pi + theta + dtheta / 2))
            )
        ] = 0
        mask[
...
def TVDerivative(img, nx, ny):
    ...
    vst1 = (2 * (fxy - fnegxy) + 2 * (fxy - fxnegy)) / (
        np.sqrt(1e-8 + (fxy - fnegxy) ** 2 + (fxy - fxnegy) ** 2)
    )
    vst2 = (2 * (fposxy - fxy)) / np.sqrt(
        1e-8 + (fposxy - fxy) ** 2 + (fposxy - fposxnegy) ** 2
    )
    vst3 = (2 * (fxposy - fxy)) / np.sqrt(
        1e-8 + (fxposy - fxy) ** 2 + (fxposy - fnegxposy) ** 2
    )
    vst = vst1 - vst2 - vst3
    vst = vst[1:-1, 1:-1]
