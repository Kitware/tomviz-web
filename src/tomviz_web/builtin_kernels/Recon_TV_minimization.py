...
def tv_derivative(recon):
    ...
    v1n = 3 * r - np.roll(r, 1, axis=0) - np.roll(r, 1, axis=1) - np.roll(r, 1, axis=2)
    v1d = np.sqrt(
        1e-8
        + (r - np.roll(r, 1, axis=0)) ** 2
    ...
    v2d = np.sqrt(
        1e-8
        + (np.roll(r, -1, axis=0) - r) ** 2
        + (np.roll(r, -1, axis=0) - np.roll(np.roll(r, -1, axis=0), 1, axis=1)) ** 2
        + (np.roll(r, -1, axis=0) - np.roll(np.roll(r, -1, axis=0), 1, axis=2)) ** 2
    )

    v3n = r - np.roll(r, -1, axis=1)
    v3d = np.sqrt(
        1e-8
        + (np.roll(r, -1, axis=1) - np.roll(np.roll(r, -1, axis=1), 1, axis=0)) ** 2
        + (np.roll(r, -1, axis=1) - r) ** 2
        + (np.roll(r, -1, axis=1) - np.roll(np.roll(r, -1, axis=1), 1, axis=2)) ** 2
    )

    v4n = r - np.roll(r, -1, axis=2)
    v4d = np.sqrt(
        1e-8
        + (np.roll(r, -1, axis=2) - np.roll(np.roll(r, -1, axis=2), 1, axis=0)) ** 2
        + (np.roll(r, -1, axis=2) - np.roll(np.roll(r, -1, axis=1), 1, axis=1)) ** 2
...
def parallelRay(Nside, pixelWidth, angles, Nray, rayWidth):
    ...
    for i in range(Nproj):  # Loop over projection angles
        ang = angles[i] * np.pi / 180.0
        ...
        for j in range(Nray):  # Loop rays in current projection
            ...
            Ix = np.logical_and(
                xx >= -Nside / 2.0 * pixelWidth, xx <= Nside / 2.0 * pixelWidth
            )
            Iy = np.logical_and(
                yy >= -Nside / 2.0 * pixelWidth, yy <= Nside / 2.0 * pixelWidth
            )
            ...
            if xx.size != 0 and yy.size != 0:
                ...
                I = np.logical_and(
                    np.abs(np.diff(xx)) <= 1e-8, np.abs(np.diff(yy)) <= 1e-8
                )
                ...
                check1 = np.logical_and(
                    b == 0, np.absolute(yrayRotated[j] - Nside / 2 * pixelWidth) < 1e-15
                )
                check2 = np.logical_and(
                    a == 0, np.absolute(xrayRotated[j] - Nside / 2 * pixelWidth) < 1e-15
                )
                ...
                if np.logical_and(numvals > 0, check):
                    ...
                    ) * Nside + (np.floor(midpoints_x / pixelWidth + Nside / 2.0))
                ...
                print(
                    ("Ray No.", j + 1, "at", angles[i], "degree is out of image grid!")
                )
    ...
    vals = vals[:idxend]
    A = ss.coo_matrix(
        (vals, (rows, cols)), shape=(Nray * Nproj, Nside**2), dtype=np.float32
    )
    return A
