...
class ReconDFMOperator(tomviz.operators.CancelableOperator):
    def transform(self, dataset):
        ...
        for array_idx, array_name in enumerate(scalars_names):
            ...
            v = pyfftw.zeros_aligned((Nx, Ny, Nz // 2 + 1), dtype="complex64", n=16)

            p = pyfftw.empty_aligned((Nx, Npad), dtype="float32", n=16)
            pF = pyfftw.empty_aligned((Nx, Npad // 2 + 1), dtype="complex64", n=16)
            ...
            for a in range(Nproj):
                ...
                self.progress.message = (
                    f"{array_name}: Tilt image No.%d/%d. " % (a + 1, Nproj) + etcMessage
                )
            ...
            v_temp = v.copy()
            recon = pyfftw.empty_aligned((Nx, Ny, Nz), dtype="float32", order="F", n=16)
