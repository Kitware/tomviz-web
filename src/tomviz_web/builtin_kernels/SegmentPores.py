...
def median_filter(operator, step_pct, input_image):
    ...
    median_filter = itk.MedianImageFilter.New(Input=input_image)
    ...
    itkutils.observe_filter_progress(
        operator, median_filter, progress, progress + next(step_pct)
    )
    try:
...
def unsharp_mask(operator, step_pct, input_image):
    ...
    enhancer = itk.UnsharpMaskImageFilter.New(Input=input_image)
    ...
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, enhancer, progress, progress + next(step_pct)
    )
    try:
        enhancer.Update()
    except RuntimeError:
...
def get_distance(operator, step_pct, input_image):
    ...
    distance_mapper = itk.SignedMaurerDistanceMapImageFilter.New(Input=input_image)
    ...
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, distance_mapper, progress, progress + next(step_pct)
    )
    try:
...
def invert(operator, step_pct, input_image):
    ...
    inverter = itk.NotImageFilter.New(Input=input_image)

    operator.progress.message = "Invert  threshold"
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, inverter, progress, progress + next(step_pct)
    )
    try:
        inverter.Update()
    except RuntimeError:
...
def threshold(operator, step_pct, input_image):
    ...
    otsu_filter = itk.OtsuMultipleThresholdsImageFilter.New(Input=input_image)
    otsu_filter.SetNumberOfThresholds(1)
    operator.progress.message = "Otsu threshold"
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, otsu_filter, progress, progress + next(step_pct)
    )
    ...
    if voxel_type is itkTypes.F or voxel_type is itkTypes.D:
        ...
        progress = operator.progress.value
        itkutils.observe_filter_progress(
            operator, caster, progress, progress + next(step_pct)
        )
...
def watershed(operator, step_pct, input_image, level):
    ...
    watershed_filter = itk.MorphologicalWatershedImageFilter[
        watershed_input_type, watershed_output_type
    ].New(Input=caster.GetOutput())
    ...
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, watershed_filter, progress, progress + next(step_pct)
    )
    try:
...
def apply_mask(operator, step_pct, input_image1, input_image2):
    ...
    mask_filter = itk.MultiplyImageFilter.New(
        Input1=caster1.GetOutput(), Input2=caster2.GetOutput()
    )
...
def morphological_closing(operator, step_pct, input_image, structuring_element):
    ...
    closer = itk.GrayscaleMorphologicalClosingImageFilter.New(Input=input_image)
    closer.SetKernel(structuring_element)
    operator.progress.message = "Morphological closing"
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, closer, progress, progress + next(step_pct)
    )
...
def encapsulate(operator, step_pct, input_image, mask, structuring_element):
    ...
    dilater = itk.GrayscaleDilateImageFilter.New(Input=input_image)
    ...
    xor_filter = itk.XorImageFilter.New(Input1=mask, Input2=dilater.GetOutput())
    progress = nextprogress
    nextprogress = progress + next(step_pct)
    itkutils.observe_filter_progress(operator, xor_filter, progress, nextprogress)

    or_filter = itk.OrImageFilter.New(Input1=input_image, Input2=xor_filter.GetOutput())
    progress = nextprogress
    nextprogress = progress + next(step_pct)
    itkutils.observe_filter_progress(operator, or_filter, progress, nextprogress)
...
def opening_by_reconstruction(operator, step_pct, input_image, structuring_element):
    import itk

    from tomviz import itkutils

    opening = itk.OpeningByReconstructionImageFilter.New(Input=input_image)
    opening.SetKernel(structuring_element)

    operator.progress.message = "Opening by reconstruction"
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, opening, progress, progress + next(step_pct)
    )
...
class SegmentPores(tomviz.operators.CancelableOperator):
    def transform(self, dataset, minimum_radius=0.5, maximum_radius=6.0):
        ...
        try:
            ...
            for dim in range(dimension):
                ...
                closing_radius[dim] = max(radius, closing_radius[dim])
            StructuringElementType = itk.FlatStructuringElement[dimension]
            structuring_element = StructuringElementType.Ball(closing_radius)
            particle_mask = morphological_closing(
                self, step_pct, thresholded, structuring_element
            )

            encapsulated = encapsulate(
                self, step_pct, thresholded, particle_mask, structuring_element
            )
            ...
            for dim in range(dimension):
                ...
                opening_radius[dim] = max(radius, opening_radius[dim])
            structuring_element = StructuringElementType.Ball(opening_radius)
            opened = opening_by_reconstruction(
                self, step_pct, in_particles, structuring_element
            )
            ...
            print("Problem encountered while running %s" % self.__class__.__name__)
            raise exc
