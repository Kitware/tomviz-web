    ...
    def transform(self, dataset, label_value=1, principal_axis=0):
        ...
        def connected_progress_func(fraction):
            self.progress.value = int(
                fraction * (STEP_PCT[1] - STEP_PCT[0]) + STEP_PCT[0]
            )
        ...
        def label_progress_func(fraction):
            self.progress.value = int(
                fraction * (STEP_PCT[2] - STEP_PCT[1]) + STEP_PCT[1]
            )
            return self.canceled

        shape_label_map = itkutils.get_label_object_attributes(
            dataset, label_progress_func
        )
        ...
        for i in range(num_label_objects):
        ...
        dataset.active_scalars = copy.deepcopy(distance.reshape(data_shape, order="F"))
