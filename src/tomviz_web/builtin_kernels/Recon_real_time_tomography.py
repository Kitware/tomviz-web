...
from tomviz._realtime import ctvlib, logger, pytvlib, wbp
...
class RealTimeTomography(tomviz.operators.CompletableOperator):
    def transform(
        ...
        tomoLogger = logger.Logger(
            localDirectory, fileExtensions[fileExt], alignments[align], invert
        )
        ...
        while runExperiment:
            ...
            for jj in range(maxIter):
                ...
                self.progress.message = (
                    "Reconstructing Tilt Angles (%d"
                    " Sampled): Iteration No.%d/%d. "
                    % (tomoLogger.logTiltAngles.shape[0], jj + 1, maxIter)
                    + etcMessage
                )
            ...
            if tomoLogger.monitor():
                ...
                self.progress.message = "Generating measurement matrix"
                pytvlib.initialize_algorithm(
                    tomo, alg, Nray, tomoLogger.logTiltAngles, 1
                )
                tomoLogger.load_tilt_series(tomo, alg)
