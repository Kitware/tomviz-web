...
class ReconConstrintedDFMOperator(tomviz.operators.CancelableOperator):
    def transform(
        ...
        for array_idx, array_name in enumerate(scalars_names):
            ...
            (recon, recon_F) = dfm3(tiltSeries, tiltAngles, np.size(tiltSeries, 1) * 2)
            ...
            for i in range(Niter):
                ...
                self.progress.message = (
                    f"{array_name}: Iteration No.%d/%d. " % (i + 1, Niter) + etcMessage
                )
...
def dfm3(input, angles, Npad):
    ...
    v = np.zeros(v.shape, dtype=np.float32) + 1j * np.zeros(v.shape, dtype=np.float32)
    recon = pyfftw.n_byte_align_empty((Nx, Ny, Ny), 16, dtype=np.float32, order="F")
    ...
    for a in range(Nproj):
        ...
        for i in range(np.int64(np.ceil(Npad / 2)) + 1):
            ky = i * dk
            # kz = 0
