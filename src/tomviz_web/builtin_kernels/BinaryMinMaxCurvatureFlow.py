...
class BinaryMinMaxCurvatureFlow(tomviz.operators.CancelableOperator):
    def transform(self, dataset, stencil_radius=2, iterations=10, threshold=50.0):
        ...
        try:
            ...
            itk_filter_image_type = itk.Image[itkTypes.F, itk_image.GetImageDimension()]
            caster = itk.CastImageFilter[
                itk_input_image_type, itk_filter_image_type
            ].New()
            caster.SetInput(itk_image)
            itkutils.observe_filter_progress(self, caster, STEP_PCT[1], STEP_PCT[2])
            ...
            smoothing_filter = itk.BinaryMinMaxCurvatureFlowImageFilter[
                itk_filter_image_type, itk_filter_image_type
            ].New()
            ...
            itkutils.observe_filter_progress(
                self, smoothing_filter, STEP_PCT[2], STEP_PCT[3]
            )
            ...
            caster = itk.CastImageFilter[
                itk_filter_image_type, itk_input_image_type
            ].New()
            caster.SetInput(itk_image_data)
            itkutils.observe_filter_progress(self, caster, STEP_PCT[3], STEP_PCT[4])
