...
class AutoTiltAxisRotationAlignOperator(tomviz.operators.CancelableOperator):
    def transform(self, dataset):
        ...
        for i in range(Nproj):
            self.progress.message = "Taking Fourier transform of tilt imageNo.%d/%d" % (
                i + 1,
                Nproj,
            )
        ...
        for a in range(coarseAngles.size):
            ...
            self.progress.message = (
                "Calculating line intensity at angle %fdegree" % (coarseAngles[a])
            )
        ...
        # now refine
        fineAngles = np.arange(
            rot_ang - coarseStep, rot_ang + coarseStep + fineStep, fineStep
        )
        ...
        for a in range(fineAngles.size):
            if self.canceled:
                return
            self.progress.message = (
                "Calculating line intensity at angle %fdegree" % (fineAngles[a])
            )
        ...
        ndimage.interpolation.rotate(tiltSeries, -rot_ang, axes=axes, output=result)
