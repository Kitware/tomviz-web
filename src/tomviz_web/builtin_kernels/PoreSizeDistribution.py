...
def make_sphere_structure(radius, ndim):
    ...
    for coord in coord_iterator(extent):
        if sum((a - b) * (a - b) for a, b in zip(coord, center, strict=False)) <= rr:
...
def get_dilation_slices(coord, shape, structure_shape):
    volume_slice = []
    structure_slice = []
    for c, r, s in zip(coord, shape, structure_shape, strict=False):
        start = c - s // 2
        stop = start + s
        start_offset = 0
        stop_offset = 0
...
def binary_dilation(a, structure):
    ...
    for coord in coord_iterator(extent):
        if a[coord]:
            a_slice, structure_slice = get_dilation_slices(
                coord, a.shape, structure.shape
            )
...
class PoreSizeDistribution(tomviz.operators.CancelableOperator):
    ...
    def transform(
        ...
        for i, r in enumerate(pore_radius):
            ...
            _, count = get_dilation_count(cavity, distance_map, r, binary_dilation)
