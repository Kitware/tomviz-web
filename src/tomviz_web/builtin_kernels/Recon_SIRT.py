...
class ReconSirtOperator(tomviz.operators.CompletableOperator):
    @apply_to_each_array
    def transform(
        self, dataset, Niter=10, stepSize=0.0001, updateMethodIndex=0, Nupdates=0
    ):
        """
        ...
        for i in range(Niter):
            ...
            for s in range(Nslice):
                ...
                self.progress.message = (
                    "Iteration No.%d/%d,Slice No.%d/%d." % (i + 1, Niter, s + 1, Nslice)
                    + etcMessage
                )
    ...
    for i in range(Nproj):  # Loop over projection angles
        ...
        for j in range(Nray):  # Loop rays in current projection
            ...
            if xx.size != 0 and yy.size != 0:
                ...
                print(
                    "Ray No. %d at %f degree is out of image grid!" % (j + 1, angles[i])
                )
