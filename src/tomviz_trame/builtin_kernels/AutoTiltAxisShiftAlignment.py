...
class AutoTiltAxisShiftAlignmentOperator(tomviz.operators.CancelableOperator):
    ...
    def transform_generate(
        ...
        if num_slices > temp.size:
            print(
                f'Warning: number of slices selected, "{num_slices}", '
        ...
        for i in range(shifts.size):
            ...
            for s in range(num_slices):
                self.progress.message = (
                    "Reconstructing slice No.%d/%d with a "
                    "shift of %d pixels" % (s, num_slices, shifts[i])
                )

                recon = wbp2(
                    shiftedTiltSeries[s, :, :], tilt_angles, Ny, "ramp", "linear"
                )
    ...
    def transform_from_file(
        self, dataset, transform_file: str = "", apply_to_all_arrays: bool = True
    ):
        ...
        axis = 1
        shift = transform_shift * transform_spacing[axis] / dataset.spacing[axis]
        return self.apply_shift_to_arrays(dataset, shift, apply_to_all_arrays)

    def apply_shift_to_arrays(self, dataset, shift, apply_to_all_arrays: bool = True):
...
def wbp2(sinogram, angles, N=None, filter="ramp", interp="linear"):
    ...
    s = np.pad(
        sinogram, ((0, F.size - Nray), (0, 0)), "constant", constant_values=(0, 0)
    )
