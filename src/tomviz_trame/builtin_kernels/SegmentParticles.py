...
def morphological_opening(operator, step_pct, input_image, structuring_element):
    ...
    opener = itk.GrayscaleMorphologicalOpeningImageFilter.New(Input=input_image)
    opener.SetKernel(structuring_element)
    operator.progress.message = "Morphological opening"
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, opener, progress, progress + next(step_pct)
    )
...
def fill_holes(operator, step_pct, input_image):
    ...
    fill_hole = itk.GrayscaleFillholeImageFilter.New(Input=input_image)
    operator.progress.message = "Fill holes"
    progress = operator.progress.value
    itkutils.observe_filter_progress(
        operator, fill_hole, progress, progress + next(step_pct)
    )
    ...
    def transform(self, dataset, minimum_radius=4):
        ...
        try:
            ...
            opened = opening_by_reconstruction(
                self, step_pct, smoothed, structuring_element
            )
            ...
            cleaned = opening_by_reconstruction(
                self, step_pct, thresholded, structuring_element
            )

            # Fill in pores
            # Grayscale implementation is faster than binary
            closed = morphological_closing(self, step_pct, cleaned, structuring_element)
            ...
            opening = morphological_opening(self, step_pct, filled, structuring_element)

            self.progress.message = "Saving results"

            label_map_dataset = dataset.create_child_dataset()
            itkutils.set_itk_image_on_dataset(
                opening, label_map_dataset, dtype=type(itk_input_image)
            )
