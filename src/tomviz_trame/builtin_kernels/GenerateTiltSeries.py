...
class GenerateTiltSeriesOperator(tomviz.operators.CancelableOperator):
    def transform(self, dataset, start_angle=-90.0, angle_increment=3.0, num_tilts=60):
        ...
        angles = np.linspace(
            start_angle, start_angle + (num_tilts - 1) * angle_increment, num_tilts
        )
        ...
        volume_pad = np.pad(
            volume,
            ((0, 0), (pad_y_pre, pad_y_post), (pad_z_pre, pad_z_post)),
            "constant",
        )
