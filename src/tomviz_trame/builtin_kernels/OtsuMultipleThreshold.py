...
class OtsuMultipleThreshold(tomviz.operators.CancelableOperator):
    def transform(self, dataset, number_of_thresholds=1, enable_valley_emphasis=False):
        ...
        try:
            ...
            otsu_filter = itk.OtsuMultipleThresholdsImageFilter[
                itk_input_image_type, itk_threshold_image_type
            ].New()
            ...
            itkutils.observe_filter_progress(
                self, otsu_filter, STEP_PCT[1], STEP_PCT[2]
            )
            ...
            if voxel_type is itkTypes.F or voxel_type is itkTypes.D:
                ...
                caster = itk.CastImageFilter[
                    itk_threshold_image_type, py_buffer_type
                ].New()
                caster.SetInput(itk_image_data)
                itkutils.observe_filter_progress(self, caster, STEP_PCT[2], STEP_PCT[3])
            ...
            itkutils.set_itk_image_on_dataset(
                itk_image_data, label_map_dataset, dtype=py_buffer_type
            )
