...
class CrossCorrelationAlignmentOperator(tomviz.operators.CancelableOperator):
    ...
    def transform_generate(self, dataset, apply_to_all_arrays, transforms_save_file):
        ...
        kFilter = (kR <= (0.5 / filterCutoff)) * np.sin(
            2 * filterCutoff * np.pi * kR
        ) ** 2
        ...
        for i in range(referenceIndex, Nproj - 1):
            if self.canceled:
                return
            self.progress.message = "Processing tilt image No.%d/%d" % (step, Nproj)
        ...
        if apply_to_all_arrays:
            names = [
                name for name in dataset.scalars_names if name != dataset.active_name
            ]
    ...
    def apply_offsets_to_scalar_names(self, dataset, offsets, names):
        ...
        for name in names:
            ...
            for i in range(len(offsets)):
                shifts = offsets[i]
                array[:, :, i] = ndimage.shift(
                    array[:, :, i], shift=shifts, order=1, mode="wrap"
                )

            dataset.set_scalars(name, array)

    def transform_from_file(self, dataset, apply_to_all_arrays, transform_file):
