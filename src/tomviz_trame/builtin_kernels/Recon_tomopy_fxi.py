...
def recon(
    ...
    rot_cen: float
        rotation center
    sli: list
        a range of slice to reconstruct, e.g. [100:300]
    bingning: int
    ...
    try:
        rec = np.zeros(
            [s[0] // binning, s[1] // binning, s[1] // binning], dtype=np.float32
        )
    except Exception:
    ...
    for i in range(n_steps):
        ...
        print(
            f"recon {i + 1}/{n_steps}:    sli = [{sli_sub[0]},", f"{sli_sub[1]}] ... "
        )
...
def wiener_denoise(prj_norm, wiener_param, denoise_flag):
    ...
    for j in range(ss[0]):
        prj_norm[j] = skr.wiener(
            prj_norm[j], psf=psf, reg=reg, balance=balance, is_real=is_real, clip=clip
        )
    return prj_norm
...
def bin_ndarray(ndarray, new_shape=None, operation="mean"):
    ...
    compression_pairs = [
        (d, c // d) for d, c in zip(new_shape, ndarray.shape, strict=False)
    ]
