...
def apply_rotation(array, rotation, spacing):
    ...
    center = [x * y / 2 for x, y in zip(array.shape, spacing, strict=False)]
...
def apply_resampling(array, spacing, reference_spacing):
    ...
    resampling_factor = [
        x / y for x, y in zip(spacing, reference_spacing, strict=False)
    ]
...
def apply_resize(array, reference_shape):
    ...
    padding = []
    cropping = []
    for x, y in zip(array.shape, reference_shape, strict=False):
        if y > x:
            total_pad = y - x
...
def transform(
    ...
    convert_to_float = (
        not all(np.isclose(x, 0) for x in rotation) and array.dtype != np.float32
    )
    if convert_to_float:
    ...
    if align_with_reference:
        array = apply_alignment(array, scaling, reference_spacing, reference_shape)
